...
function openbank(title)
    ...
    if Config.sellgold then 
        table.insert(elements, {label = Config.language.sellgold, value = 'sellgold' , desc = Config.language.sellgold2}) 
    end
   MenuData.Open('default', GetCurrentResourceName(), 'menuapi',
	...
	function(data, menu)
        ...
        if(data.current.value == 'sellgold') then
            TriggerEvent("syn_inputs:sendinputs", Config.language.confirm, Config.language.amount, function(cb)
                local amount =     tonumber(cb)
                if amount ~= nil and amount > 0 then
                   TriggerServerEvent("syn_bank:sellergold",amount,bank)
                else
                    TriggerEvent("vorp:TipBottom", Config.language.invalid, 6000) 
                    inmenu = false 
                end
