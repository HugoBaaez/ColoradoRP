                ...
                if listed then 
                    ...
                    if Citizen.InvokeNative(0xC92AC953F0A982AE,stashanimalprompt) then
                        ...
                        if Config.maxanimals > wagoncount then
                            Citizen.InvokeNative(0xC7F0B43DCDC57E3D, player, heldentity, coords, 10.0, 1)
                            local isskin = false 
                            if quality then 
                                isskin = true 
                            end
                            TriggerServerEvent("syn_clan:stashanimal",animalnetwork,wagonserverid,outfit,model,isskin,humanbody)
                            Wait(1500)
                        ...
                        if v.functional == "hunting" then 
                            if next(supplymissionactivewagons) ~= nil and IsPedInAnyVehicle(PlayerPedId()) then 
                                ...
                                if PlayerPedId() == driver then 
                                    ...
                                    if vehnetworkcontained(supplymissionactivewagons,network) then
                    ...
                    if getinmenu() then 
                        ...
                        elseif allowlockpick and radiedclanid ~= nil and radiedclanid == activeclanid then 
                            ...
                            if haslockpickitem then
                                ingame = true  
                                local res = exports['SS-Lockpick']:Lockpick()
								if res then 
                                    allowlockpick = false 
                                    lockpicktimer = nil 
...
function checkstashmenu() -- find me 
    ...
    for k,v in pairs(wagoninfo.stash) do 
        ...
        table.insert(elements, {label = nameofanimal, value = v.animalnetwork , desc =  language.takeoutanimal,type = v.isskin } )
    end
   MenuData.Open('default', GetCurrentResourceName(), 'menuapi',
	{
	...
	function(data, menu)
        ...
        if DoesEntityExist(animal) then
            while not NetworkHasControlOfEntity(animal)  do
                Wait(100)
                NetworkRequestControlOfEntity(animal)
            end
            SetEntityVisible(animal, true)
            DetachEntity(animal, 0, false)
            FreezeEntityPosition(animal, false)
            SetEntityCollision(animal, true, true)
            SetEntityCoords(animal, GetOffsetFromEntityInWorldCoords(closewagon, 0.0, -3.0, 0.0))
        end
                ...
                if Config.locationdistancecheck > dist then 
                    if next(deliverywagonsactive) ~= nil and IsPedInAnyVehicle(PlayerPedId()) then 
                        ...
                        if PlayerPedId() == driver then
                            ...
                            local network = VehToNet(wagon)
                            if vehnetworkcontained(deliverywagonsactive,network) then
