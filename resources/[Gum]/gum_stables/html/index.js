    ...
    window.addEventListener('message', function(event) {
        ...
        if (item.type === "open_stable") {
            if (item.status == true) {
                ...
                if (callMeFrom == true || item.callMeHorseStable == true || item.callMeCartStable == true) {
                ...
                if (callMeFrom == true || callMeHorseStable == true || callMeCartStable == true) {
                    if (transferEnable == true) {
        ...
        if (item.type === "comp_cart_load") {
            ...
            extras = item.extras
            MakeExtras(extras, item.max_lantern, item.max_livery, item.max_tints, id)
            in_menu = 6
            $("#MenuApi").hide();
            $(".grid5").show();
        }
...
function HorseSpots() {
    ...
    let dataHtml = '';
    if (callMeHorseStable == true) {
        for (var a in myhorses) {
            if (myhorses[a].cityPosition !== 0) {
                dataHtml += '<div class="horse_store_button2" id="horse_store_button" onclick="thisHorseTransfer('+myhorses[a].id+')">'+stablesInformation[(myhorses[a].cityPosition-1)].name+' </br> '+myhorses[a].name+'</div>'
            } else {
                dataHtml += '<div class="horse_store_button2" id="horse_store_button" onclick="thisHorseTransfer('+myhorses[a].id+')">'+spotsInformation[(myhorses[a].spotPosition-1)].name+' </br> '+myhorses[a].name+'</div>'
            }
        }
    }
    if (callMeCartStable == true) {
        for (var a in mywagons) {
            if (mywagons[a].cityPosition !== 0) {
                dataHtml += '<div class="horse_store_button2" id="horse_store_button" onclick="thisCartTransfer('+mywagons[a].id+')">'+stablesInformation[(mywagons[a].cityPosition-1)].name+' </br> '+mywagons[a].name+'</div>'
            } else {
                dataHtml += '<div class="horse_store_button2" id="horse_store_button" onclick="thisCartTransfer('+mywagons[a].id+')">'+spotsInformation[(mywagons[a].spotPosition-1)].name+' </br> '+mywagons[a].name+'</div>'
            }
        }
    }
    if (callMeFrom) {
    ...
    let data2Html = '';
    if (callMeFrom == true || callMeHorseStable == true) {
        for (var a in stablesInformation) {
            data2Html += '<div class="horse_store_button" id="horse_store_button" onclick="thisCityTransfer('+stablesInformation[a].position+')">'+stablesInformation[a].name+'</div>'
        }
    }
    if (callMeFrom == true || callMeCartStable == true) {
        for (var a in spotsInformation) {
            data2Html += '<div class="horse_store_button" id="horse_store_button" onclick="thisSpotTransfer('+spotsInformation[a].position+')">'+spotsInformation[a].name+'</div>'
        }
    }
    table2Body.innerHTML = data2Html
}

function thisHorseTransfer(id) {
    $.post('http://gum_stables/thisHorseTransfer', JSON.stringify({
        id: id
    }));
}

function thisCartTransfer(id) {
    $.post('http://gum_stables/thisCartTransfer', JSON.stringify({
        id: id
    }));
}

function thisCityTransfer(pos) {
    $.post('http://gum_stables/thisCityTransfer', JSON.stringify({
        pos: pos
    }));
}

function thisSpotTransfer(pos) {
...
function LoadCartData(table_inv) {
    ...
    for (var a in table_inv) {
        if (latest[table_inv[a].category] === undefined) {
            ...
            data2Html += '<div class="button_category"><div class="r_arrow_button" onclick="right_cart(\''+table_inv[a].category+'\')"></div><div id="buy_horse" onclick="buycart(\''+table_inv[a].data[1]+'\',\''+table_inv[a].category+'\')"></div><u>'+table_inv[a].category+'</u><div id="'+table_inv[a].category+'" class="button_horse">'+table_inv[a].data[1]+'</div></div>'
        }
    }
    table2Body.innerHTML = data2Html
}
...
function MakeExtras(table_inv, lantern, livery, tints, id) {
...
function BuyComponents() {
    ...
    $.post('http://gum_stables/BuyComponents', JSON.stringify({blanket:blanket,saddle:saddle,horns:horns,stirrup:stirrup,bags:bags,bedroll:bedroll,lantern:lantern,accessorie:accessorie,tails:tailes,manes:manes,mustache:mustache,holster:holster,bridle:bridle}));
}
var isOver = false
...
function LoadMyHorsesData(table_inv) {
    ...
    for (var a in table_inv) {
        if (callHorse) {
            if (table_inv[a].breeding == "Breeding") {
...
function AddBreed(id) {
    $.post('http://gum_stables/add_to_breed', JSON.stringify({id:id}))
}
function Save_breed_horse(sex, model, id1, id2, time) {
    $.post('http://gum_stables/save_breed', JSON.stringify({sex:sex, model:model, id1:id1, id2:id2, time:time}))
}
