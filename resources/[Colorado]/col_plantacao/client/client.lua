...
AddEventHandler('poke_planting:planto1', function(hash1, hash2, hash3, seed)
    ...
    for k,v in pairs(Config.Locations) do
        ...
        if GetDistanceBetweenCoords(v.x, v.y, v.z, pos.x, pos.y, pos.z, true) < 30.0 and canPlant then
            ...
            if seed ~= "sementemilho" or seed == "sementemilho" then
                if myPlants then
...
AddEventHandler('poke_planting:regar1', function(source)
    ...
    if canWater then
        ...
        if DoesEntityExist(object) then
            ...
            myPlants[#myPlants+1] = {["object"] = object, ['x'] = x, ['y'] = y, ['z'] = z, ['stage'] = 2, ['timer'] = 300, ['hash'] = hash1, ['hash2'] = hash2, ['hash3'] = hash3}
            local plantCount = #myPlants
            PlaceObjectOnGroundProperly(myPlants[plantCount].object)
            SetEntityAsMissionEntity(myPlants[plantCount].object, true)
        end
		...
		if myPlants[1] ~= nil and nearField then
			...
			for k, v in ipairs(myPlants) do
				if GetDistanceBetweenCoords(v.x, v.y, v.z, pos.x, pos.y, pos.z, true) < 7.0 then
					...
					if v.stage == 2 then
						DrawText3D(v.x, v.y, v.z, 'Em Crescimento: ' .. v.timer)
					end
					if v.stage == 3 then
						DrawText3D(v.x, v.y, v.z, 'Pronto para ser recolhido! [E]')
					end
					if v.prompt then
...
function animacion()
    ...
    exports['progressBars']:startUI(10000, 'Rastrelando...')
    Wait(10000)
    ClearPedTasksImmediately(PlayerPedId())
	Wait(1000)
