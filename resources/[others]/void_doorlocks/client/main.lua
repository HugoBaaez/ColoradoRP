...
Citizen.CreateThread(function()
    while true do
        ...
        for k,doorID in ipairs(Config.DoorList) do
            ...
            local maxDistance, displayText = 1.25, 'abrir'
...
AddEventHandler('void_doorlocks:opendoor', function(lockbreak)
    ...
    for k,doorID in ipairs(Config.DoorList) do
        ...
        if  distance <= maxDistance and lockbreak == true and doorID.locked == false then
            TriggerServerEvent("void_doorlocks:notify", "A porta já está aberta !")
...
AddEventHandler('void_doorlocks:updatedoor', function(source, door, lockbreak)
    if not lockbreak then
        ...
        local testplayer = exports['SS-Lockpick']:Lockpick()
        if testplayer then 
