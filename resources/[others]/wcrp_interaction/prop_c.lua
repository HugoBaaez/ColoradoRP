...
function FPrompt(text, button, hold)
    Citizen.CreateThread(function()
        ...
        PropPrompt=nil
        local str = text or "Jogue fora" --Put Away
...
AddEventHandler('prop:ledger', function() 
    FPrompt("Jogue fora", 0x3B24C470, false)
...
AddEventHandler('prop:cigarettes', function() 
    ...
    EPrompt("Mudar postura", 0xD51B784F, false)
    --ExecuteCommand('close')
    local ped = PlayerPedId()
    local male = IsPedMale(ped)
    local x,y,z = table.unpack(GetEntityCoords(ped, true))
    ...
    TriggerEvent('stress:modify', -7.0)
    if male then
...
AddEventHandler('prop:cigar', function()
    ...
    local boneIndex = GetEntityBoneIndexByName(ped, 'SKEL_R_Finger12')
    local smoking = false
    TriggerEvent('stress:modify', -15.0)
...
AddEventHandler('prop:pipe', function() 
    FPrompt("Jogue fora", 0x3B24C470, false) --Put Away
    LMPrompt("Usar", 0x07B8BEAF, false) --Use
    EPrompt("Mudar postura", 0xD51B784F, false) --Pose
    --ExecuteCommand('close')
    local ped = PlayerPedId()
    local male = IsPedMale(ped)
    local x,y,z = table.unpack(GetEntityCoords(ped, true))
   ...
   AttachEntityToEntity(pipe, ped, righthand, 0.005, -0.045, -0.003, -170.0, 10.0, -20.0, true, true, false, true, 1, true)
