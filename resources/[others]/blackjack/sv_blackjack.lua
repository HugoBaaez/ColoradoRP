   ...
   local _source = source
    ...
    local money2 = money*2
		if condition == "lose" then
			...
			TriggerClientEvent("vorp:TipRight", playerId, "Dealer ganhou, você perdeu$" ..money.. "", 5000) -- from server side
            SendWebhookMessage(wbhk,"```prolog\n[ROLETA]: ".. condition .." \n[ "..playername.. " perdeu " ..money.."] "..os.date("\n[Data]: %d/%m/%Y [Hora]: %H:%M:%S").." \r```") 
		elseif condition == "win" then
		--	VORP.addCurrency(playerId, 0, money*2)
			TriggerClientEvent("vorp:TipRight", playerId, "Você ganhou $" ..money2.. "", 5000) -- from server side
            SendWebhookMessage(wbhk,"```prolog\n[ROLETA]: ".. condition .." \n[ "..playername.. " ganhou " ..money2.."] "..os.date("\n[Data]: %d/%m/%Y [Hora]: %H:%M:%S").." \r```") 
            Character.addCurrency(0,  money2)
		elseif condition == "push" then
			--VORP.addCurrency(playerId, 0, money)
			TriggerClientEvent("vorp:TipRight", playerId, "Empate! Você recuperou seu dinheiro", 5000) -- from server side
            SendWebhookMessage(wbhk,"```prolog\n[ROLETA]: ".. condition .." \n[ "..playername.. " ganhou " ..money2.."] "..os.date("\n[Data]: %d/%m/%Y [Hora]: %H:%M:%S").." \r```") 
