    ...
    while true do
        ...
        for _, menuConfig in pairs(menuConfigs) do
            ...
            if menuConfig:enableMenu() then
                ...
                if IsControlJustReleased(0, keybindControl) and not IsEntityDead(PlayerPedId()) then
                    -- Init UI
                    showMenu = true
                    SendNUIMessage({
                        type = 'init',
...
RegisterCommand('banjo', function(souce, args, rawCommand)
    TaskStartScenarioInPlace(PlayerPedId(), GetHashKey("PROP_HUMAN_SEAT_CHAIR_BANJO"), 200000, true, false, false, false)
end, false
)
RegisterCommand('bandolin', function(souce, args, rawCommand)
    TaskStartScenarioInPlace(PlayerPedId(), GetHashKey("PROP_HUMAN_SEAT_BENCH_MANDOLIN"), 200000, true, false, false, false)
end, false
)
RegisterCommand('violao', function(souce, args, rawCommand)
    TaskStartScenarioInPlace(PlayerPedId(), GetHashKey("PROP_HUMAN_SEAT_CHAIR_GUITAR"), 200000, true, false, false, false)
end, false
)
RegisterCommand('sanfona', function(souce, args, rawCommand)
    TaskStartScenarioInPlace(PlayerPedId(), GetHashKey("PROP_HUMAN_SEAT_BENCH_CONCERTINA"), 200000, true, false, false, false)
end, false
)
RegisterCommand('trompete', function(souce, args, rawCommand)
    TaskStartScenarioInPlace(PlayerPedId(), GetHashKey("WORLD_HUMAN_TRUMPET"), 200000, true, false, false, false)
end, false
)
RegisterCommand('gaita', function(souce, args, rawCommand)
    TaskStartScenarioInPlace(PlayerPedId(), GetHashKey("PROP_HUMAN_SEAT_BENCH_HARMONICA"), 200000, true, false, false, false)
end, false
)
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(0)
        if (IsControlJustPressed(0,0x8CC9CD42)) and IsInputDisabled(0)  then -- X
            local ped = PlayerPedId()
