...
Citizen.CreateThread(function()
	while true do
		...
		if not Config.RaiseAnimal then
			if currentPetPed and not Retrieving and HuntMode then --Checking to see if your pet is active, not retriving and not hungry
			...
			if currentPetPed and not Retrieving and petXP >= Config.FullGrownXp and HuntMode then --Checking to see if your pet is active, not retriving and not hungry
				local ped = PlayerPedId()
				local ClosestPed = GetClosestAnimalPed(ped,Config.SearchRadius)
				local pedType = GetPedType(ClosestPed)		  			
				if pedType == 28 and IsEntityDead(ClosestPed) and not RetrievedEntities[ClosestPed] then
        ...
        if IsPlayerTargettingAnything(id) then
			...
			if PromptHasStandardModeCompleted(HuntModePrompt[entity]) then
					if not HuntMode then
						TriggerEvent( 'UI:DrawNotification', "Seu pet está pronto para caçar")
						HuntMode = true
					else
					HuntMode = false
					TriggerEvent( 'UI:DrawNotification', "Seu pet não está pronto para caçar")
					end					
			end			
...
RegisterCommand("rpet", function(source, args, rawCommand) --  COMMAND
...
RegisterCommand("cpet", function(source, args, rawCommand) --  COMMAND
    local _source = source
    local ped = PlayerPedId()
	TriggerServerEvent('rdn_companions:loaddog')
