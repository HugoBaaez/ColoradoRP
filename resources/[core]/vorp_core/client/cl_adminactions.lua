...
--===========================================================================================================================--
RegisterNetEvent('vorp:TeleportToPlayer', function(coords)
    local ped = PlayerPedId()
    local height = 1

    for height = 1, 1000 do
        SetEntityCoords(ped, coords.x, coords.y, coords.z, height + 0.0)
        local foundground, groundZ, normal = GetGroundZAndNormalFor_3dCoord(coords.x, coords.y, coords.z, height + 0.0)
        if foundground then
            SetEntityCoords(ped, coords.x, coords.y, coords.z, height + 0.0)
...
RegisterNetEvent('vorp:TeleportToCoords', function(x, y, z)
    local ped = PlayerPedId()
    local height = 1

    for height = 1, 1000 do
        SetEntityCoords(ped, x, y, z, height + 0.0)
        local foundground, groundZ, normal = GetGroundZAndNormalFor_3dCoord(x, y, z, height + 0.0)
        if foundground then
            SetEntityCoords(ped, x, y, z, height + 0.0)
