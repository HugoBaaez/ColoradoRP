...
local function runSQLList(list, type)
    for index, it in ipairs(list) do
        ...
        if it.find then
            local isfound = exports.oxmysql:executeSync(it.find)
            if #isfound > 0 then
                hascolumn = true
        ...
        if hascolumn == false then
            local result = exports.oxmysql:executeSync(it.sql)
...
function RunDBCheck()
    local rversion = exports.oxmysql:executeSync('SELECT VERSION();')
