            ...
            if _source ~= 0 then -- its a player
                ...
                if CheckAceAllowed(Config.AcePerms, _source) or CheckGroupAllowed(Config.GroupAllowed, group) then -- check ace first then group
                    if CurrentCommand == "addGroup" then
                        ...
                        VorpCore.NotifyRightTip(_source, "Você adicionou o grupo para o ID: " .. target, 4000)
                        VorpCore.NotifyRightTip(_source, "STAFF adicionou você no grupo " .. newgroup, 4000)
                    ...
                    elseif CurrentCommand == "addJob" then
                        ...
                        VorpCore.NotifyRightTip(_source,
                            "Você setou o serviço " .. newjob .. " para o ID " .. target .. " Com o cargo" .. jobgrade, 4000)
                        VorpCore.NotifyRightTip(target, "A STAFF te setou no serviço " .. newjob .. " Com o cargo " .. jobgrade, 4000)
                    ...
                    elseif CurrentCommand == "addMoney" then
                        ...
                        VorpCore.NotifyRightTip(_source, "Você enviou " .. quantity .. " para o ID " .. target, 4000)
                        VorpCore.NotifyRightTip(target, "Você recebeu da STAFF o valor de " .. quantity, 4000)
                        ...
                        if itemCheck then
                            if canCarry then
                                if canCarry2 then
                                    ...
                                    VorpCore.NotifyObjective(_source, "Você não consegue carregar mais desse item", 4000)
                                end
                            else
                                VorpCore.NotifyObjective(_source, "Inventário cheio", 4000)
                            end
                        end
                    ...
                    elseif CurrentCommand == "delMoney" then
                        ...
                        VorpCore.NotifyRightTip(_source, "Você removeu " .. quantity .. " do ID " .. target, 4000)
                    ...
                    elseif CurrentCommand == "reviveplayer" then
                        ...
                        if #args == 0 or target == _source then
                            ...
                            if VorpCore.getUser(target) then
                                ...
                                VorpCore.NotifyObjective(_source, "ID incorreto ou player inexistente", 4000)
                            end
                        end
                    ...
                    elseif CurrentCommand == "banplayer" then
                        ...
                        if _source ~= target then -- cant ban yourself
                            ...
                            if not CheckGroupAllowed(Config.GroupAllowed, Group) then -- bann only non staff players
                                ...
                                if banTime == 0 then
                                    datetime = 0
                                    text = "Foi banido permanentemente"
                                elseif banTime then
                    ...
                    elseif CurrentCommand == "tpcds" then
                        local x = tonumber((args[1]:gsub(",",""))) + .0
                        local y = tonumber((args[2]:gsub(",",""))) + .0
                        local z = tonumber((args[3]:gsub(",",""))) + .0
                        if x ~= 0 and y ~= 0 and z ~= 0 then
                            TriggerClientEvent('vorp:TeleportToCoords', _source, x, y, z)
                            if Config.Logs.TpmWebhook then
                                local Message = "`\n**PlayerID** `" .. _source .. "`\n **Action:** `Used TPCDS` para o local".. x.. y.. z 
                                local title = "📋` /TPCDS command` "
                                VorpCore.AddWebhook(title, Config.Logs.TpmWebhook, message .. Message)
                            end
                        else
                            TriggerClientEvent("vorp:ShowBottomRight", _source, "Formato de Coordenadas Incorreto", 5000)
                        end
                    elseif CurrentCommand == "tpp" then
                        local target = GetPlayerPed(tonumber(args[1]))
                        if target ~= 0 then
                            local coords = GetEntityCoords(target)
                            TriggerClientEvent('vorp:TeleportToPlayer', _source, coords)
                            if Config.Logs.TpmWebhook then
                                local Message = "`\n**PlayerID** `" .. _source .. "`\n **Action:** `Used TPP para o player `"..target
                                local title = "📋` /TPP command` "
                                VorpCore.AddWebhook(title, Config.Logs.TpmWebhook, message .. Message)
                            end
                        else
                            TriggerClientEvent("vorp:ShowBottomRight", _source, "ID não existe", 5000)
                        end
