...
InventoryAPI.canCarryAmountItem = function(player, amount, cb)
	...
	local maxInventory = Config.MaxItemsInInventory.Items

	if sourceCharacter.group == "Prata" then
		maxInventory = 110	
	elseif sourceCharacter.group == "Bronze" then
		maxInventory = 105  
	elseif sourceCharacter.group == "Ouro" then
		maxInventory = 150  
	elseif sourceCharacter.group == "Diamante" then
		maxInventory = 175  
	elseif sourceCharacter.group == "Platina" then
		maxInventory = 185
	end
	if userInventory and maxInventory ~= -1 then
		local sourceInventoryItemCount = InventoryAPI.getUserTotalCount(identifier) + amount
		if sourceInventoryItemCount <= maxInventory then
			cb(true)
		else
			cb(false)
		end
...
InventoryAPI.canCarryItem = function(player, itemName, amount, cb)
	...
	if limit ~= -1 then
		...
		if maxInventory ~= -1 then
			...
			if totalAmount <= maxInventory then
				cb(true)
			else
				cb(false)
			end
