...
local wbhk = "https://discord.com/api/webhooks/991436409686736927/iznIgJdZUMtwEyAy83-v1RX-AvDZOfcJ5IXnjRFMctBTdZafMb9UO-NYyMZYq6ftWkMX"
...
AddEventHandler("syn_weapons:addcomp", function(weaponid,added)
    ...
    local playername = Character.firstname.. ' ' ..Character.lastname.. ' Com Emprego ' ..Character.job
    ...
    if weaponid ~= nil then
        ...
        SendWebhookMessage(wbhk,"```prolog\n[COMPONENTES]: ".. playername.." \n Adicionou componentes a arma com ID:"..weaponid.." componentes: "..components.." "..os.date("\n[Data]: %d/%m/%Y [Hora]: %H:%M:%S").." \r```")
    end
end)
function getTime ()
...
AddEventHandler("syn_weapons:checkweapon", function(id,target)
    Citizen.Wait(1000)
    exports.oxmysql:execute('SELECT identifier FROM loadout WHERE id = @id ' , {['id'] = id}, function(result)
        if result[1] ~= nil then 
            ...
            if identifier2 == "" then
                ...
                exports.oxmysql:execute("UPDATE loadout Set identifier=@identifier WHERE id=@id", {['identifier'] = identifier,['id'] = id})
                exports.oxmysql:execute("UPDATE loadout Set charidentifier=@charidentifier WHERE id=@id", {['charidentifier'] = charidentifier,['id'] = id})
            end
        end
    end)
end)
...
AddEventHandler("syn_weapons:removeused", function(data)
  ...
  exports.oxmysql:execute("UPDATE loadout Set used=@used AND used2=@used2 WHERE identifier=@identifier AND charidentifier = @charidentifier", Parameters) 
end)

RegisterServerEvent("syn_weapons:cleanup")
AddEventHandler("syn_weapons:cleanup", function()
    exports.oxmysql:execute("DELETE FROM loadout WHERE dropped=@dropped", { ['dropped'] = 1})
end)
...
exports.oxmysql:execute("UPDATE loadout Set dropped=@dropped WHERE id=@id", Parameters)
end)
...
AddEventHandler("syn_weapons:removeallammoserver", function()
    ...
    local charidentifier = Character.charIdentifier
    exports.oxmysql:execute('SELECT ammo FROM characters WHERE charidentifier = @charidentifier ' , {['charidentifier'] = charidentifier}, function(result)
		local ammo = json.decode(result[1].ammo)
        if next(ammo) ~= nil then 
            local Parameters = { ['charidentifier'] = charidentifier, ['ammo'] = json.encode({})}
            exports.oxmysql:execute("UPDATE characters Set ammo=@ammo  WHERE charidentifier=@charidentifier", Parameters)    
    ...
    exports.oxmysql:execute('SELECT ammo FROM characters WHERE charidentifier = @charidentifier ' , {['charidentifier'] = charidentifier}, function(result)
        ...
        if contains then 
            ...
            --    TriggerClientEvent("syn_weapons:givebackbox",_source,item)
                TriggerClientEvent('vorp:TipRight', _source, 'Você tem o máximo de munições!', 3000)
...
AddEventHandler("syn_weapons:givebackbox", function(item)
    local _source = source
    VorpInv.addItem(_source, item, 3)
end)
RegisterServerEvent("syn_weapons:givebackbox1")
AddEventHandler("syn_weapons:givebackbox1", function(item)
    local _source = source
    VorpInv.addItem(_source, item, 1)
end)
